...
class NepseTradingBot:
    ...
    async def start(self) -> None:
        ...
        # Start IPO early staging task
        self._tasks.append(asyncio.create_task(self._ipo_staging_orchestrator()))
    ...
    async def _ipo_staging_orchestrator(self) -> None:
        """
        Background task to coordinate staging and trigger at market open (11:00 AM)
        for IPO daily circuit watchlist items.
        """
        # 1. Find the IPO symbol from watchlist
        ...
        if not ipo_symbol or not ipo_item:
            logger.info("ipo_staging_no_active_ipo_symbols_found")
            return

        # 2. Calculate target circuit limit price
        prev_close = ipo_item.prev_close
        if prev_close <= 0:
            logger.warning("ipo_staging_invalid_prev_close", symbol=ipo_symbol, prev_close=prev_close)
            return

        circuit_pct = ipo_item.circuit_percentage or 15.0
        import math
        target_price = math.floor(prev_close * (1 + circuit_pct / 100.0) * 10) / 10

        # Quantity profile resolution
        quantity = 10 # default fallback
        if self.strategy_engine:
            strat = self.strategy_engine.get_strategy("ipo_daily_circuit")
            if strat:
                qty_profile = strat.parameters.get("quantity_profile", "default")
                profile = self.strategy_engine._quantity_profiles.get(qty_profile, {"type": "fixed", "quantity": 10})
                if profile.get("type") == "fixed":
                    quantity = int(profile.get("quantity", 10))
                max_qty = self.settings.risk_max_quantity_per_order
                quantity = min(quantity, max_qty)

        logger.info(
            "ipo_staging_orchestrator_initialized",
            symbol=ipo_symbol,
        ...
        while self._running:
            ...
            if not staged:
                is_staging_window = (
                    (now_nepal.hour == 10 and now_nepal.minute >= 50) or
                    (now_nepal.hour == 10 and now_nepal.minute < 59)
                )
                ...
                if is_staging_window or is_test_run:
                    logger.info("ipo_staging_triggering_form_stage", symbol=ipo_symbol)
                    if hasattr(self.broker, "stage_order"):
                        staged = await self.broker.stage_order(ipo_symbol, quantity, target_price)
                        if staged:
                            self.staged_flag = True
                            logger.info("ipo_staging_form_staged_successfully", symbol=ipo_symbol)
                        else:
                            logger.error("ipo_staging_form_stage_failed", symbol=ipo_symbol)
                    else:
            ...
            pct_change = 0.0
            if self.market_monitor:
                tick = self.market_monitor.get_latest_tick(ipo_symbol)
                if tick:
                    if tick.ltp > 0:
            ...
            if staged and not triggered:
                ...
                if current_sec % 5 == 0 and current_sec != last_logged_sec:
                    ...
                    if current_price == 0.0:
                        logger.info("waiting_for_first_price_tick", symbol=ipo_symbol)
                    else:
                        logger.info(
            ...
            if staged and not triggered:
                ...
                if should_trigger:
                    logger.info("ipo_staging_starting_fast_trigger_loop", symbol=ipo_symbol, time=current_time_str)
                    triggered = True
                    
                    # Pre-resolve order tokens and session data once to eliminate Playwright API latency in the loop
                    scrip_id = None
                    exchange = None
                    cookies = None
                    user_agent = None
                    
                    if not self.simulate and hasattr(self.broker, "_page") and self.broker._page:
                        try:
                            scrip_id = await self.broker._page.evaluate("Selected_scrip")
                            exchange = await self.broker._page.evaluate("Selected_Exchange")
                            ...
                            user_agent = await self.broker._page.evaluate("navigator.userAgent")
                            logger.info("pre_resolved_order_tokens_successfully", scrip_id=scrip_id, exchange=exchange)
                        except Exception as e:
                            logger.warning("failed_to_pre_resolve_order_tokens", error=str(e))
                    ...
                    while attempts < max_attempts and self._running and not success:
                        attempts += 1
                        logger.info("fast_trigger_attempt", attempt=attempts, time=datetime.now(tz).strftime("%H:%M:%S.%f"))
                        ...
                        if hasattr(self.broker, "fast_trigger_buy"):
                            res = await self.broker.fast_trigger_buy(
                                ...
                                quantity=quantity,
                                price=target_price,
                                kill_switch=self.settings.risk_kill_switch,
                                scrip_id=scrip_id,
                                exchange=exchange,
                                cookies=cookies,
                            ...
                            if res.get("success"):
                                ...
                                logger.debug("fast_trigger_attempt_failed", attempt=attempts, reason=reason, message=message)
                                if reason == "Kill switch active":
                                    logger.warning("fast_trigger_stopped_due_to_kill_switch")
                                    break
                        # Constant high-frequency sleep (50ms) to ensure maximum speed once the 8% price trigger is hit
                        await asyncio.sleep(0.05)
                    
                    if not success:
                        logger.error("ipo_staging_fast_trigger_loop_exhausted_without_success")
                        last_failed_price = current_price  # Record the price level where we failed
